...
def _clear_cached_values():
    ...
    listener._get_bootstrap_servers.cache_clear()
    ...
    # Reset the module-level producer cache.
    listener._producer = None
    listener._producer_retry_after = 0.0
...
def kafka_producer_mock():
    """Patches the confluent_kafka.Producer class."""
    mock = MagicMock()
    ...
    with patch("confluent_kafka.Producer", return_value=mock):
        yield mock
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            [],
            id="no_bootstrap_servers",
        ),
        pytest.param(
            {
                ("kafka_listener", "dag_run_events_enabled"): "True",
                ("kafka_listener", "task_instance_events_enabled"): "True",
                ("kafka_listener", "bootstrap_servers"): "broker:29092",
            },
...
class TestGetProducer:
    ...
    def test_topic_doesnt_exist(self, monkeypatch):
        ...
        with patch("confluent_kafka.Producer", return_value=kafka_producer_mock):
        ...
        warning_format, topic_arg, bootstrap_arg = log_warning_mock.call_args.args
        assert "topic %r not found on broker %r" in warning_format
        assert topic_arg == _KAFKA_TOPIC
        assert bootstrap_arg == "broker:29092"
    ...
    def test_atexit_register(self, monkeypatch):
        ...
        register_mock.assert_called_once_with(listener._flush_producer, kafka_producer_mock)

    def test_producer_cached_on_success(self, monkeypatch):
        time_now = [1000.0]
        ...
        with patch("confluent_kafka.Producer", return_value=kafka_producer_mock) as producer_cls_mock:
            producer1 = listener._get_producer()
            # Advance well past the retry interval.
            time_now[0] += 10 * listener._get_topic_check_retry_interval()
            # Once the producer is successfully initialized, the value is cached and
            # `_producer_retry_after` is set to `float("inf")`, making the interval check
            # always fail and resulting in the existing value getting returned.
    ...
    def test_retried_after_failure_cooldown(self, monkeypatch):
        ...
        kafka_producer_mock = MagicMock()
        # The topic doesn't exist. Producer will fail to initialize.
        ...
        with patch("confluent_kafka.Producer", return_value=kafka_producer_mock) as producer_cls_mock:
            ...
            # The interval hasn't passed and the producer initialization hasn't been retried.
            time_now[0] += 1
            assert listener._get_producer() is None
            assert producer_cls_mock.call_count == 1

            # Past the interval. Producer initialization is retried.
...
class TestFilters:
    ...
    def test_is_allowed(self, id_to_check, allowlist, denylist, expected_result):
        assert listener._is_allowed(id_to_check, allowlist, denylist) is expected_result
